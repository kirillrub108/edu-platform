...
### 6.4 Дублирование логики доступа к уроку (частично закрыто)
...
- **Что было не так:** проверка «студент записан» была продублирована inline в нескольких роутерах, помимо `require_lesson_access`. Пока правилом был один `SELECT` по `Enrollment`, это грозило только N+1 и расхождением поведения; с появлением избирательного доступа (DECISIONS §62) любая пропущенная копия стала **дырой в access-контроле** — отозванный студент продолжал бы ходить по этому пути.
- **Что сделано (2026-08-31):** само правило доступа вынесено в [services/course_access_service.py](../backend/app/services/course_access_service.py) (`get_enrollment` / `has_access` / `access_clause`) и подключено во всех точках, где `Enrollment` служил гейтом: `dependencies.require_lesson_access` и `require_course_access`, `routers/students.py` (`enroll`, `my-courses`, `courses/{id}`, `get_lesson_for_student`, `_get_progress`, дашборд и списки), `routers/quiz_student.py:_ensure_enrolled`, `services/assignment_service.get_published_assignment_for_student`, рассылка в `services/comment_service.py`.
- **Что осталось:** дублируется не правило, а **обвязка** — каждый роутер по-прежнему сам грузит `Lesson` → `Module` → зовёт сервис и сам решает, отвечать 403 или 404. Это лишние запросы и риск разъехавшихся кодов ответа, но уже не обход проверки.
- **Фикс:** свести обвязку к одной зависимости (`require_lesson_access`) в `routers/students.py` отдельной задачей. **Инвариант:** любой новый student-facing путь обязан спрашивать доступ через `course_access_service`, а не через прямой `SELECT` по `Enrollment`.
...
### — ~~`Course.owner` может прийти `None`~~ — ЗАКРЫТО (2026-08-29)

- **Где:** [backend/app/schemas/course.py](../backend/app/schemas/course.py).
- **Было:** `CourseOut.owner: UserOut` (обязательный) падал при сериализации курсов
  soft-delete-нутого преподавателя, которого скрывает глобальный фильтр.
- **Почему перестало быть гипотетическим:** появился эндпоинт самоудаления
  (`POST /users/me/delete`), а purge теперь **обезличивает** юзера вместо удаления, поэтому строка
  остаётся `deleted_at IS NOT NULL` **навсегда**, а не 30 дней.
- **Фикс:** `owner: UserOut | None = None` — вариант, предложенный здесь же. Вторая альтернатива
  («каскадно архивировать курсы препода») отвергнута: отзывает доступ у записанных студентов, что
  прямо противоречит [DECISIONS §51](DECISIONS.md). Фронт рисует «Удалённый пользователь» +
  инициалы (`components/UserAvatar.vue`), ссылка на профиль ведёт в 404. См.
  [DECISIONS §59.6](DECISIONS.md).
...
### — Текстовые уроки ускоряют упор в лимит 2 ГБ на урок, а дерево базы знаний курса без пагинации

- **Где:** [backend/app/services/lesson_material_service.py](../backend/app/services/lesson_material_service.py) (`register_uploaded_file`, `get_course_knowledge`), [backend/app/routers/uploads.py](../backend/app/routers/uploads.py), [backend/app/constants.py](../backend/app/constants.py) (`LESSON_MATERIAL_MAX_INLINE_FILES`). Контекст решения — `DECISIONS.md` §61, курсовое дерево — §64.
- **Что не так (1) — авто-регистрация удваивает расход лимита.** `/uploads/pptx` и `/uploads/script` теперь дополнительно кладут СВОЮ копию файла в `materials/{lesson_id}/` (см. §61: общий объект связал бы «удалить материал» с поломкой генерации). Значит PPTX занимает место дважды — под `pptx/` и под `materials/`. Наложено на предыдущий пункт («нет ретеншна, нет потолка на курс/аккаунт») это ускоряет упор в лимит 2 ГБ на урок примерно вдвое для уроков со слайдами.
- **Что не так (2) — inline-картинки едят тот же бюджет.** Картинки в теле текстового урока — это те же `LessonMaterial` и тот же лимит 30 файлов / 2 ГБ. Подлимит `LESSON_MATERIAL_MAX_INLINE_FILES` (20) ограничивает только их количество, не объём: 20 изображений по 50 МБ (потолок категории `image`) = 1 ГБ, половина бюджета урока. Учителю это видно — панель «Файлы» показывает строку вида «Использовано 24 из 30 файлов · 412 МБ из 2048 МБ · из них 18 — вложения в тексте урока», и то же разложение попадает в текст ошибки 400, — но самого потолка на курс/аккаунт по-прежнему нет.
- **Что не так (3) — дерево базы знаний курса отдаётся целиком.** `GET /courses/{id}/knowledge` возвращает модули → уроки → материалы одним ответом без пагинации. Тела конспектов из него исключены намеренно (`CourseKnowledgeNoteRead` не содержит `content`) — это снимает главный вклад, — но метаданные материалов остаются: курс на 100 уроков по 30 файлов = 3000 записей с подписанными URL в одном JSON. Поиск на этой странице — клиентский, по уже загруженному дереву, что ровно та же ставка. Чинить — пагинация по модулям либо серверный поиск, когда появится курс, на котором это заметно.
- **Что не так (4) — сироты чистятся только при сохранении тела.** `save_text_body` удаляет inline-материалы, на которые в новом тексте не осталось ссылок. Если преподаватель удалил картинку из текста и НЕ сохранил (ушёл со страницы), файл остаётся числиться за уроком; отдельного сборщика по расписанию нет — как и в пункте (3) предыдущей записи.
- **Почему сейчас не блокер:** те же лимиты на урок остаются в силе, `purge_pipeline` по-прежнему метёт весь `materials/{lesson_id}` при хард-удалении, а типичный курс сегодня — десятки уроков, а не сотни.
...
### — `lessons.duration_sec` хранит длительность только последней генерации

Урок держит **историю** видео (`lesson_videos`, каждый прогон — своя строка), а поле
длительности добавлено на `Lesson`
([DECISIONS §57](DECISIONS.md#57-целевая-длительность-урока--бюджет-слов-а-не-скорость-tts-2026-08-26)).
Следствие: после повторной генерации в интерфейсе видна длительность нового ролика, даже
если преподаватель оставил опубликованным старый, а для старых видео из истории
длительность не показывается вовсе. Чинится переносом поля на `LessonVideo` (плюс
бэкфилл `ffprobe` по существующим файлам). Не сделано: на текущем UI длительность нужна
только рядом с целевой, а история показывает даты и режим, а не тайминги.

### — Поштучная регенерация слайдов не фиксирует версию брифа

`lesson.narration_brief` ([DECISIONS §63](DECISIONS.md#63-бриф-озвучки-пожелания-автора-как-данные-в-user-сообщении-а-не-как-правило-системного-промпта-2026-09-08))
читается из строки урока в момент запроса — и при полном анализе, и при
`POST /lessons/{id}/slides/{slide_id}/regenerate`. Если преподаватель поменял бриф и
перегенерировал только пару слайдов, урок оказывается собран под две разные версии
пожеланий, и по текстам этого не видно. Осознанный размен: альтернатива — хранить бриф на
прогоне анализа (колонка на `SlideText` или отдельная сущность «прогон») и подставлять
регенерации именно ту версию, что и остальным слайдам. Не сделано: ради согласованности
одного необязательного поля заводить сущность прогона дороже, чем предупредить в UI, что
после смены брифа анализ нужно запустить заново.

### — Темп речи 130 слов/мин задан константой в двух местах

`WORDS_PER_MINUTE` живёт и в `backend/app/constants.py`, и в
`frontend/src/composables/useLessonDuration.ts` — фронт считает оценку длительности
локально, чтобы она обновлялась по мере набора текста, без запроса к API. Значение
одинаковое, но синхронизируется руками. Плюс сама цифра — среднее по больнице: реальный
темп зависит от голоса и провайдера TTS, и после генерации его можно было бы измерять
(`duration_sec / words`) и уточнять. Не сделано: точность оценки не стоит калибровочного
контура.

### — Осиротевшая колонка `lessons.target_duration_min`

Целевая длительность заменена на `lessons.detail_level`
([DECISIONS §57](DECISIONS.md#57-глубина-раскрытия-темы-вместо-целевой-длительности-2026-08-27)),
но миграция `f4e9b6f10146` колонку **не дропает**: правило expand/contract
([DECISIONS §53](DECISIONS.md)) запрещает удалять в том же релизе, где предыдущая версия
кода ещё пишет в колонку, и `app/scripts/migration_guard.py` это проверяет в CI. Колонка
больше не читается и не пишется ни моделью, ни схемами. Дроп — отдельной миграцией в
следующем релизе, после того как `bfd6e82` полностью выкачен.

### — `users.email` не имеет регистронезависимой уникальности

Уникальный индекс стоит на `email` как есть, поэтому `A@x.com` и `a@x.com` — две разные строки,
и **обычная парольная регистрация** (`AuthService.register`, точное сравнение `User.email == email`)
не помешает завести вторую такую строку.

**Что при этом НЕ ломается:** OAuth-линковка (ветка B, [DECISIONS §58](DECISIONS.md)) сравнивает
по нормализованному адресу — `func.lower(User.email) == normalize_email(...)` — и находит юзера при
любом регистре с обеих сторон. Оба направления зафиксированы тестами
(`test_callback_links_existing_email_and_verifies_it` — заглавные у провайдера,
`test_callback_links_mixed_case_row_in_db` — заглавные в БД). Второй аккаунт на ту же почту
социальный вход не создаёт.

**Что остаётся:** (1) поиск идёт мимо индекса — нет functional index на `lower(email)`, seq scan на
каждом социальном входе (на текущем объёме незаметно); (2) если пара `A@x.com` / `a@x.com` уже
образовалась через парольную регистрацию, `scalar()` молча возьмёт одну из двух.

**Решение отложено сознательно:** закрывается миграцией «нормализовать существующие адреса в lower +
`CREATE UNIQUE INDEX ... ON users (lower(email))` + нормализация на входе в `register`», но шаг
нормализации требует продуктового решения, что делать с уже существующей коллизией (какой из двух
аккаунтов победит, куда девать его курсы и записи), — это не часть OAuth-фичи. До тех пор
инвариант держится кодом, а не схемой.

### — Обложка курса грузится без проверки сигнатуры

- **Где:** [backend/app/routers/courses.py](../backend/app/routers/courses.py) (`upload_course_cover`),
  [backend/app/routers/uploads.py](../backend/app/routers/uploads.py) (`upload_cover`).
- **Что не так:** оба обработчика проверяют только расширение и размер и **не зовут**
  `file_validation_service.validate_upload`, поэтому SVG (или любой другой байт-поток),
  переименованный в `.png`, доезжает до хранилища, а `/files/*` отдаёт его инлайном — хранимый XSS,
  тот же вектор, что закрыт для аватара.
- **Что уже сделано:** ветка изображений в `_check_magic` добавлена (JPEG/PNG/WebP/GIF), то есть
  проверка существует и работает — её просто никто не вызывает на пути обложки. Аватар
  (`POST /users/me/avatar`) через неё проходит.
- **Фикс:** позвать `validate_upload(file, _COVER_EXTS, enforce_size_limits=False)` в обоих
  обработчиках. Не сделано в рамках задачи про профили — это отдельный путь загрузки, менять его
  поведение заодно значило бы расширить диff за пределы задачи. См. [DECISIONS §59.4](DECISIONS.md).
- **Дополнительно:** обложка при замене **не удаляет** старый файл (`cover_image_path`
  перезаписывается), поэтому `storage/covers/` копит сироты. У аватара этой проблемы нет — там
  сносится весь префикс `avatars/{user_id}/`.

### — `storage/avatars/` не подчищается автоматически

- **Где:** [backend/app/services/profile_service.py](../backend/app/services/profile_service.py)
  (`store_avatar`, `drop_avatar_file`), [backend/app/tasks/purge_pipeline.py](../backend/app/tasks/purge_pipeline.py)
  (`gc_disk_caches`).
- **Что не так:** `avatars/` не входит в ночной `gc_disk_caches` (как и `tts_cache`/`tts_chunk_cache`).
- **Почему почти не проявляется:** замена и `DELETE` аватара сносят весь префикс
  `avatars/{user_id}/` перед записью, а обезличивание вызывает `drop_avatar_file`, поэтому сирота
  появляется только при падении процесса между `delete_prefix` и `save_bytes`.
- **Фикс по запросу:** добавить в `gc_disk_caches` проход по `avatars/`, сверяя пути с
  `users.avatar_image_path`.

### — Чужой курс в `routers/courses.py` отвечает 403, а не 404

`_get_owned_course` отдаёт `404 Course not found` только на несуществующий `course_id`, а на чужой —
`403 Not your course`. Это подтверждает существование курса произвольному преподавателю по его UUID.
Соседнее правило видимости (`dependencies.require_lesson_access`) прячет недоступное под 404 именно
чтобы не раскрывать существование ресурса. Затрагивает все teacher-роуты курса, включая новый импорт
списка доступа (§65 в `DECISIONS.md`), который переиспользует ту же проверку. Утечка небольшая (нужен
угаданный UUID), чинится однострочно — заменой 403 на 404 в `_get_owned_course`, но это меняет
контракт сразу у десятка эндпоинтов и их тестов, поэтому вынесено отдельно.

### — Ссылка на удаление аккаунта уходит без проверки доставляемости адреса

- **Где:** [backend/app/services/account_service.py](../backend/app/services/account_service.py)
  (`request_account_deletion`), [backend/app/routers/auth.py](../backend/app/routers/auth.py)
  (`POST /auth/request-delete`).
- **Что не так:** соседний `resend-verification` перед отправкой зовёт
  `email_deliverability_service.assert_deliverable` (подавление + MX), а запрос ссылки на удаление —
  нет. Письмо уйдёт и на адрес, который уже в `email_suppressions`.
- **Почему не критично:** обратный вариант хуже. Отказ на подавленном адресе означал бы, что
  пользователь с забаунсившей почтой не может удалить аккаунт вообще — единственный фактор
  подтверждения теперь именно почта (§67 в `DECISIONS.md`). Лишняя отправка на мёртвый ящик — один
  bounce, а не потеря доступа.
- **Фикс по запросу:** предупреждать в интерфейсе (флаг баунса уже есть в `MeOut`) и предлагать
  сначала сменить адрес через `POST /auth/change-email`, не блокируя сам запрос ссылки.
