...
| **База знаний урока** | `models/lesson_material.py` (`LessonMaterial` + `LessonNote`), `routers/lesson_materials.py` | Преподаватель прикладывает файлы (методички) и пишет markdown-конспекты к уроку; студент видит их на вкладке урока. Без AI. Один роутер на обе сущности, гейты — уже существующие (`require_lesson_access`/`get_owned_lesson`); файлы только хранятся, свой лимит (30/2 ГБ на урок), без ретеншна. См. [DECISIONS.md](DECISIONS.md) §47. |
