...
считают ожидаемый объём как `slides × AUTO_CHARS_PER_SLIDE × (words[level] / words[auto])`.
...
Manual-режим (`presentation_and_text`) параметра не имеет вовсе: там текст авторский, LLM
его только режет по слайдам и размечает SSML. Показывается лишь оценка по фактическому
тексту.
...
- **Резать/дописывать авторский текст в manual-режиме.** Преподаватель приносит свой текст
  именно потому, что хочет свой текст.
