...
## 57. Целевая длительность урока — бюджет слов, а не скорость TTS (2026-08-26)

**Контекст:** преподаватель не мог задать длину урока. Единственный «рычаг» длительности,
который выглядел дешёвым, — `speed` у SpeechKit: подогнать любой текст под любую цифру,
меняя темп чтения. Отдельный вопрос — что делать с режимом `presentation_and_text`, где
текст лекции пишет человек.

**Решение:** `lessons.target_duration_min` (nullable, произвольное число минут в диапазоне
`LESSON_DURATION_MIN_MINUTES`…`LESSON_DURATION_MAX_MINUTES` = 1…180, пресеты 5/10/15/20/30 —
только кнопки в UI; `NULL` = «авто, по объёму текста»). Длительность реализуется как
**бюджет слов**: `target × WORDS_PER_MINUTE`, распределённый по слайдам
([duration_service.slide_word_budgets](../backend/app/services/duration_service.py)), уезжает
в промпт нарратива как явное ограничение объёма
([vision_analysis.SLIDE_BUDGET_TEMPLATE](../backend/app/services/vision_analysis.py)) и
перекрывает дефолтные «150–300 слов» системного промпта. Скорость TTS не трогается.

Бюджет **взвешенный, не равномерный**: титульный и заключительный слайды получают
`EDGE_SLIDE_BUDGET_WEIGHT` (0.4) от доли содержательного слайда — веса нормируются, поэтому
сумма всегда равна цели. При 1–2 слайдах взвешивание вырождается в равное деление.

**Тарификация зависит от цели.** Урок на 5 и на 50 минут не может стоить одинаково: озвучка
длиннее, и vision-анализ пишет больше текста. `estimate_video_auto(slides, target)` и
`estimate_vision_analyze(slides, target)` считают ожидаемый объём как
`target × WPM × CHARS_PER_WORD`, где `CHARS_PER_WORD = 7` — **замер** по ~22 тыс. слов
реально сгенерированной озвучки (6.96), а не оценка. Без цели формулы возвращают прежние
значения (`slides × AUTO_CHARS_PER_SLIDE`), так что старые уроки не переоцениваются.
Триал free-плана сверяет ожидаемый объём с тем же `TRIAL_MAX_SCRIPT_CHARS`, что и текстовый
режим, — иначе произвольная длительность превращала бесплатный слот в безлимит на TTS.

**Цель применяется на этапе анализа презентации, не генерации видео.** В auto-режиме
`generate-video` переиспользует уже написанные `SlideText`, поэтому смена цели без
повторного анализа ничего не меняет — селектор длительности стоит на шаге «Презентация»
перед кнопкой анализа, и UI об этом предупреждает явно.

В режиме `presentation_and_text` авторский текст **не переписывается и не режется**:
цель используется только для оценки (`words / WPM`), и при расхождении больше
`DURATION_MISMATCH_RATIO` (25%) преподаватель получает предупреждение в
`generation-estimate` / ответе `generate-video`. Генерацию это не блокирует.

Промах LLM по бюджету — не ошибка: пайплайн не падает, статус урока не `error`, ретраев
из-за длительности нет. Фактическая длительность меряется `ffprobe` после конкатенации и
пишется в `lessons.duration_sec` — рядом с целевой в интерфейсе.

**Альтернативы:**
- **Подгонять `speed` у TTS.** Отклонено: портит восприятие (ускоренная речь читается как
  дефект), работает только у Yandex-провайдера (`silero`/`polza` игнорируют `speed`, см. §15),
  и маскирует настоящую проблему — неверный объём текста.
- **Резать/дописывать авторский текст под цель.** Отклонено: преподаватель приносит свой
  текст именно потому, что хочет свой текст. Молчаливое редактирование хуже честного
  предупреждения.
- **Итеративная догенерация до попадания в цель.** Отклонено: цикл LLM-вызовов на каждый
  слайд умножает и стоимость, и латентность ради точности, которой никто не просил.
- **Хранить длительность на `LessonVideo`, а не на `Lesson`.** Отклонено как избыточное для
  задачи; ограничение записано в [KNOWN_PROBLEMS](KNOWN_PROBLEMS.md).
- **Фиксированный набор длительностей (5/10/15/20/30) вместо произвольного ввода.** Была
  первая итерация; отклонено — преподавателю нужна своя цифра, а набор остался пресетами.
- **Равномерный бюджет по слайдам.** Отклонено: титульный слайд с той же нормой, что и
  содержательный, заставляет модель лить воду.

**Trade-offs:** попадание в цель приблизительное — LLM соблюдает бюджет слов неточно, а
темп речи (`WORDS_PER_MINUTE = 130`) — константа, а не измерение конкретного голоса
(замер на одной лекции дал ~140). Отдельно: цель делится на **фактическое** число слайдов,
поэтому короткая дека под длинную цель даёт бюджет, который vision-модель физически не
вытянет (3 слайда × 15 мин = 1083 слова на содержательный слайд против дефолтных 150–300).
Это не ошибка пайплайна — недобор просто виден в оценке слайд-редактора.
`WORDS_PER_MINUTE` продублирован во фронтовом `useLessonDuration.ts` — оценка в
`SlideTextEditor` считается локально, без запроса к API; при изменении константы нужно
править оба места.
