...
57. [Глубина раскрытия темы вместо целевой длительности](#57-глубина-раскрытия-темы-вместо-целевой-длительности-2026-08-27)
58. [OAuth-вход: PKCE + state в Redis, pending-тикет вместо создания юзера в callback](#58-oauth-вход-pkce--state-в-redis-pending-тикет-вместо-создания-юзера-в-callback-2026-08-28)
59. [Lifecycle аккаунта: soft-delete → restore → освобождение email → обезличивание вместо DELETE; модель аватара](#59-lifecycle-аккаунта-soft-delete--restore--освобождение-email--обезличивание-вместо-delete-модель-аватара-2026-08-29)
60. [Клиентский кулдаун после 429 на /auth/register и /auth/login](#60-клиентский-кулдаун-после-429-на-authregister-и-authlogin-2026-08-30)
61. [Текстовый урок: тело в `Lesson.text_content`, inline-вложения — те же `LessonMaterial`, ссылка `material:{uuid}` вместо подписанного URL](#61-текстовый-урок-тело-в-lessontext_content-inline-вложения--те-же-lessonmaterial-ссылка-materialuuid-вместо-подписанного-url-2026-08-31)
62. [Избирательный доступ к курсу: режим `invite` поверх существующего `access_mode`, гранты как отдельная таблица](#62-избирательный-доступ-к-курсу-режим-invite-поверх-существующего-access_mode-гранты-как-отдельная-таблица-2026-08-31)
63. [Бриф озвучки: пожелания автора как данные в user-сообщении, а не как правило системного промпта](#63-бриф-озвучки-пожелания-автора-как-данные-в-user-сообщении-а-не-как-правило-системного-промпта-2026-09-08)
64. [Курсовая база знаний: read-only агрегат поверх поурочных сущностей](#64-курсовая-база-знаний-read-only-агрегат-поверх-поурочных-сущностей-2026-09-09)
...
## 63. Бриф озвучки: пожелания автора как данные в user-сообщении, а не как правило системного промпта (2026-09-08)

**Контекст:** `detail_level` (§57) отвечает на вопрос «сколько текста», но не на вопрос
«про что и как». Преподаватели просили о втором: указать аудиторию («9 класс»), акценты,
терминологию («английские термины произносить по-русски») и то, чего касаться не нужно.
Единственный способ это сделать сегодня — вручную править каждый `SlideText` после анализа.

**Решение:** `lessons.narration_brief` (Text, nullable, лимит `NARRATION_BRIEF_MAX_CHARS`
= 1000), свободный текст, пишется тем же `PUT /lessons/{id}`, что и уровень раскрытия;
пустая строка нормализуется схемой в NULL. Бриф уходит vision-LLM **как данные**:
в user-сообщении отдельным блоком `<author_brief>…</author_brief>`, а в системный промпт
добавляется одна секция, которая явно называет иерархию — пожелания влияют на содержание,
акценты, аудиторию и терминологию, но **не меняют язык вывода, формат ответа и бюджет слов
на слайд**. Это тот же приём, что и `_SSML_DETAIL_OVERRIDE` в §57: инструкция, отменяющая
другое правило, обязана назвать, какое правило проигрывает.

**Почему данные, а не инструкция системе.** Бриф пишет пользователь, и он не отличим от
попытки перехватить формат («ответь одним словом», «верни JSON», «пиши по-английски»).
Склеенный с системным промптом, такой текст встаёт вровень с правилами вывода и роняет
пайплайн — озвучка на другом языке, «ответ» вместо текста, пустой слайд. В user-сообщении
он остаётся содержимым, о котором система заранее сказала: это данные, не команда.
Фильтровать текст мы не пытаемся (чёрные списки формулировок обходятся тривиально) —
защиту даёт разметка и приоритет правил.

**Отсутствие брифа не меняет ничего.** Без брифа блок в user-сообщении и секция в системном
промпте отсутствуют **физически**, а не «пустые»: `_system_prompt(None) is
VISION_SYSTEM_PROMPT`, и тест фиксирует это побайтовым сравнением. Дефолтный сценарий
обязан остаться ровно тем, чем был.

**Только auto-режим.** Бриф читает vision-LLM, а vision-LLM работает только в
`presentation_auto`. В manual-режиме автор уже написал текст — направлять модель незачем,
её роль там сводится к сжатию/дополнению этого текста (§57). Поэтому бриф применяется на
этапе **анализа**, читается воркером из строки урока (сигнатура
`analyze_presentation_task.delay(lesson_id, pptx_path)` не расширена — иначе retry сработал
бы со старым брифом), и по той же причине его получает регенерация одного слайда: иначе
пересобранный слайд выпадал бы из заданного направления.

**База знаний урока не подмешивается.** Соблазн скормить vision-LLM `LessonMaterial`
(«вот учебник, ориентируйся на него») отвергнут: по §47 материалы принципиально только
хранятся и никогда не парсятся сервером — парсинг вернул бы XXE/zip-бомбы, а объём
непредсказуем и лёг бы в каждый слайдовый запрос. Бриф — ручной, короткий и ограниченный
1000 символами эквивалент того же намерения.

**Биллинг не меняется.** `estimate_vision_analyze` / `estimate_video_auto` /
`detail_ratio` не тронуты: объём по-прежнему функция `detail_level`, а бриф — функция
содержания. Именно поэтому секция промпта отдельно запрещает брифу двигать бюджет слов:
иначе «расскажи подробнее про примеры» тихо увеличила бы TTS-счёт мимо оценки, показанной
преподавателю до запуска.

**Альтернативы:**
- *Набор структурированных полей (аудитория / уровень / стоп-темы).* Форма, которую
  заполняют один раз и больше не открывают; половина реальных пожеланий («не пересказывать
  формулы вслух») ни в одно поле не влезает. Компромисс — 5 пресетов-чипов над свободным
  полем: типовое кладётся в один клик, нетиповое пишется руками.
- *Бриф в системном промпте.* См. выше — открывает пользовательскому тексту доступ к
  правилам формата.
- *Отдельный эндпоинт `PUT /lessons/{id}/brief`.* Лишний роут ради одной nullable-колонки;
  `LessonUpdate` уже проходит через `get_owned_lesson`.
- *Отдельная карточка в UI.* Дублировала бы предупреждение «применяется во время анализа»;
  свёрнутая строка внутри карточки «Степень раскрытия темы» оставляет дефолтный путь тем же
  по числу кликов.

**Trade-offs:** (а) бриф **не переписывает** уже сгенерированные `SlideText` — после его
смены нужен повторный анализ, ровно как с уровнем раскрытия (UI предупреждает тем же
текстом). (б) Слайды, регенерированные поштучно в разное время, могут оказаться сделаны под
разные версии брифа — бриф читается из урока в момент запроса, а не фиксируется на прогоне
(KNOWN_PROBLEMS). (в) Гарантий исполнения нет: это подсказка модели, а не контракт;
проверить, что «9 класс» действительно учтён, можно только глазами. (г) Лимит в 1000
символов выбран как компромисс между полнотой и тем, что бриф уходит в **каждый** слайдовый
запрос и оплачивается токенами на каждом слайде.

**Связано:** §57 (глубина раскрытия — вторая ось той же ручки), §47 (материалы урока не
парсятся), §13 (vision вместо OCR).


---

## 64. Курсовая база знаний: read-only агрегат поверх поурочных сущностей (2026-09-09)

**Проблема.** База знаний из §47 живёт на уровне урока. Чтобы собрать всё, что приложено к курсу,
преподавателю приходилось заходить в каждый урок, а студент вообще не имел способа увидеть материалы
курса одним списком. N вызовов `GET /lessons/{id}/knowledge` — это N круговых поездок и N подписей.

**Решение.** Один эндпоинт `GET /api/v1/courses/{course_id}/knowledge`, отдающий дерево
модуль → урок → (материалы, метаданные конспектов). Новых сущностей, колонок и миграций нет:
материал по-прежнему привязан к уроку, `lesson_id` остаётся `NOT NULL`, а «курсовая вкладка» — это
чистая агрегация на чтение плюс переиспользование существующих write-эндпоинтов
`/lessons/{id}/…` с выбором целевого урока в UI.

*Хендлер лежит в `routers/courses.py`, а не в `lesson_materials.py`.* Префикс `/api/v1/lessons`
делят три роутера, и одинаковые пути там молча затеняются порядком регистрации; префикс
`/api/v1/courses` принадлежит одному роутеру. Логика при этом общая — тот же вызов
`lesson_material_service.get_course_knowledge`, роутер остаётся тонким.

*Гейт — существующая зависимость `require_course_access`* (владелец-преподаватель ИЛИ студент с
доступом), возвращающая `(user, course, is_owner)`. Новую писать не пришлось. Незаписанный студент
получает **403**, а не 404 — курс существует и виден в каталоге, скрывать нечего; правило «черновик
прячется под 404» относится к неопубликованным ресурсам, а не к отсутствию записи, и 403 здесь
совпадает с соседним `routers/students.py:course_details`. `can_edit` в ответе — решение сервера
(`is_owner`), фронт роль не угадывает.

*Срез видимости — через `visibility_service`, не инлайном.* Для студента дерево прунится
`module_visible_to_student` / `lesson_visible_to_student`: черновой модуль и черновой урок не
попадают в ответ **вообще**, даже пустым заголовком, поэтому API не выдаёт факт их существования.
Владелец получает всё, но каждый модуль и урок несёт `is_published`, чтобы UI пометил черновик.
Публикация самого курса в правило не входит (§34): архивированный курс с сохранённым доступом
продолжает отдавать свою базу знаний записанному студенту.

*Тела конспектов в агрегат не входят.* `CourseKnowledgeNoteRead` — это метаданные без `content`.
При лимитах §47 (до 100 конспектов на урок × 50 000 символов) курс на сотню уроков превратил бы
один ответ в десятки мегабайт. Тело подгружается поурочно существующим
`GET /lessons/{id}/knowledge` — точечный эндпоинт для одного конспекта заводить не понадобилось.

*Ссылки на файлы в агрегате остаются.* `serialize_material` подписывает каждый материал через
`storage_service.get_url`; на локальном хранилище это внутрипроцессный HMAC — считанные микросекунды
на объект, поэтому дешевле отдать URL сразу, чем заводить второй запрос на скачивание. На S3 это
тоже локальный подсчёт подписи, без сетевого вызова. Ставка зафиксирована в `KNOWN_PROBLEMS.md`
вместе с отсутствием пагинации.

*Один компонент на оба кабинета.* `components/knowledge/CourseTree.vue` рисует дерево, а раскрытый
урок монтирует уже существующий `KnowledgePanel` — тот сам грузит поурочную базу знаний (с телами
конспектов) и владеет всеми путями записи. Поэтому преподаватель загружает файл и правит конспект
прямо из курсовой вкладки без единого нового write-эндпоинта, а студент видит ту же вкладку
в режиме чтения — потому что сервер вернул ему `can_edit=false`. Счётчики в свёрнутой строке берутся
из поурочного стора, если урок уже раскрывали, поэтому после загрузки файла курс не перезапрашивается.

**Альтернативы.**

- *Сущность `CourseMaterial` рядом с `LessonMaterial`* — отвергнута: вторая модель, миграция, второй
  контур валидации, второй префикс в хранилище и вторая строка в `purge_pipeline` ради данных,
  которые содержательно те же самые.
- *`lesson_id` nullable + материалы «курса вообще»* — отвергнута: превращает NOT NULL-инвариант в
  условный, ломает `require_lesson_access` как единственный гейт и заставляет каждую выборку по
  уроку думать о NULL.
- *Отдавать `content` конспектов в дереве* — отвергнута по объёму ответа (см. выше).
- *Пагинация дерева* — отвергнута пока: при текущих лимитах (30 файлов на урок) реального курса,
  на котором это заметно, ещё нет. Записано в `KNOWN_PROBLEMS.md`, а не сделано авансом.
- *Отдельный студенческий каталог из `pages/student/courses/[id].vue`* — отвергнут: этот файл
  legacy-редирект на первый урок; раздел добавлен как свой роут `[courseId]/knowledge.vue` с пунктом
  в сайдбаре `layouts/student.vue`, поэтому у вкладки есть свой URL и она переживает переход между
  уроками.

**Trade-offs.**

- **+** Ноль миграций, ноль новых write-путей, поурочный контракт не тронут.
- **+** Одно дерево на оба кабинета; редактирование гейтится сервером, а не ролью на клиенте.
- **−** Урок без материалов и конспектов всё равно показывается строкой с пометкой «пусто» — так
  преподаватель видит, куда можно приложить файл; для студента такой урок остаётся видимым
  заголовком, если сам урок опубликован.
- **−** Ответ не пагинируется, поиск на странице клиентский по уже загруженному дереву.

**Связано:** §47 (база знаний урока), §61 (inline-вложения, почему хендлер в `courses.py`),
§34 (правило видимости), §51 (архивированный курс сохраняет доступ).

## 65. Массовый импорт списка доступа: серверный парсинг файла, один эндпоинт с выбором колонки (2026-09-09)

**Проблема.** Режим «Только по списку» (§62) добавляет студентов по одному адресу. Преподаватель
с группой в 30 человек получает 30 отправок формы, а исходный список у него уже есть — выгрузкой
из деканата или Excel-таблицей.

**Решение.** Один эндпоинт `POST /api/v1/courses/{course_id}/access-grants/import`, multipart:
`file` + необязательный `email_column`. Разбор `.xlsx` (openpyxl) и `.csv` синхронный, вызывается
из async-роута через `run_in_threadpool`. В ответе — построчный отчёт: номер строки как в Excel,
исходное значение, нормализованный адрес, `added`/`skipped` и машинный код причины
(`invalid_email`, `duplicate_in_file`, `already_in_list`, `is_course_owner`, `not_registered`).
Человекочитаемые формулировки рисует фронт, CSV-отчёт собирается на клиенте из того же JSON.

*Парсинг на сервере, а не в браузере.* Клиентский разбор xlsx требует новой npm-зависимости
(SheetJS и аналоги), а npm в этом репозитории запрещён — зависимости живут только внутри
Docker-образа. На бэкенде цена вопроса — один `openpyxl`.

*Один эндпоинт с `needs_column_choice`, а не пара preview + apply.* Колонка определяется по
заголовку (`email`/`почта`/…), иначе по единственной колонке с валидными адресами. Неоднозначность
(две колонки с почтой или ни одной) — не ошибка, а ответ 200 с пустым отчётом, списком колонок и
примерами значений; в БД при этом ничего не пишется. Повторный запрос приходит с тем же файлом и
`email_column`. Пара эндпоинтов потребовала бы держать распарсенный файл между вызовами — то есть
серверное состояние ради шага, который стоит одного повторного разбора двухмегабайтного файла.

*Второго пути записи в allowlist нет.* Каждый адрес проходит через тот же
`course_access_service.grant_access`, что и одиночное «Добавить», и через тот же валидатор email
(схема `CourseAccessGrantCreate`). Поведение по незарегистрированным адресам, дублям и владельцу
курса наследуется как есть и только отражается в отчёте отдельными кодами. Цена — по одному
коммиту на студента (`grant_access` коммитит сам); при лимите в 1000 строк это приемлемо.

*Не Celery.* Разбор файла в 2 МБ и до 1000 строк укладывается в один HTTP-запрос, а отчёт нужен
преподавателю немедленно. Фоновая задача добавила бы очередь, polling и хранение файла ради
операции, которая идёт секунды.

*Лимиты* (`ROSTER_IMPORT_MAX_FILE_BYTES` 2 МБ, `ROSTER_IMPORT_MAX_ROWS` 1000,
`ROSTER_IMPORT_SAMPLE_VALUES` 3) — в `constants.py`. Превышение размера → 413, битый или чужой
формат → 400, превышение числа строк → 400 с фактическим количеством (не молчаливая обрезка).
Тип файла определяется по расширению И сигнатуре содержимого — `content_type` из браузера не
доверенный источник. Файл нигде не сохраняется: разбирается в памяти и отбрасывается.

**Trade-offs.** Импорт атомарен по строке, а не по файлу: частичный успех — норма, и повторная
загрузка того же файла идемпотентна (`already_in_list`). Незарегистрированные адреса не
приглашаются письмом — это ограничение унаследовано от одиночного добавления, а не введено здесь.

## 66. Обратная связь о доставке почты: подавление по вебхуку Resend, MX-проверка на регистрации, смена email (2026-09-10)

**Контекст.** Заметная доля писем «Подтвердите ваш email» уходила в Bounced. Аккаунт об этом не
узнавал (сообщить по почте нельзя — она и не доставляется), а регистрация с заведомо мёртвых
адресов вроде `casdsa@dsa.da` не блокировалась. Три дыры: нет приёма статусов доставки, нет
сигнала владельцу, нет отсева на входе.

**Решение.**

*Приём статусов.* `POST /api/v1/webhooks/resend` (`routers/webhooks_email.py`) — тонкий роутер по
образцу вебхука ЮKassa: проверка Svix-подписи на stdlib `hmac` (без SDK), быстрый 200, вся работа
в sync-таске `handle_email_delivery_event` на очереди `celery_email`. `email.bounced` с
`bounce.type = hard` и `email.complained` пишут строку в `email_suppressions` и проставляют
`users.email_bounced_at` / `email_bounce_reason`; soft-bounce только логируется; `email.delivered`
снимает флаг. Пустой `RESEND_WEBHOOK_SECRET` = 503, а не 404: Resend ретраит 5xx и не ретраит 404,
так что события переживают окно «выкатили код, секрет ещё не проставили».

*Две независимые идемпотентности.* Ретрай ТОГО ЖЕ события (svix-id) — полный no-op через Redis
`SET NX`, как одноразовость verify-токена. Новый bounce на тот же адрес — `hits + 1` и обновление
`occurred_at`. Смешивать нельзя: ретрай — это не второй отказ. Сама запись идёт одним
`INSERT ... ON CONFLICT DO UPDATE`: worker обрабатывает очередь параллельно, и на
select-then-insert два события об одном адресе, пришедшие вместе, роняли одно из них на уникальном
индексе (воспроизведено на живом воркере, не гипотеза).

*Отсев на регистрации.* `email_deliverability_service.assert_deliverable` — подавление, затем DNS:
MX, при отсутствии — A/AAAA (implicit MX, RFC 5321 §5.1). Не Pydantic-валидатор: нужна сеть.
Асинхронный резолвер (`dnspython`), домен приводится к punycode перед резолвом, вердикт кешируется
в Redis по домену. **Fail-open**: только NXDOMAIN — авторитетное «имени не существует» — считается
отказом; таймаут и SERVFAIL means «неизвестно» и пропускают регистрацию с WARNING. Ронять
регистрацию из-за своей инфраструктуры нельзя.

*Смена email.* `POST /auth/change-email` шлёт подтверждение на НОВЫЙ адрес; до перехода по ссылке
не меняется ничего. Токен — та же схема `itsdangerous` с отдельной солью `email-change`, в payload
`{uid, old, new}`. `old` — это то, что закрывает гонку «смена + подтверждение старого токена»:
токен привязан к адресу, который был у аккаунта в момент выпуска, поэтому любой предыдущий токен
перестаёт подходить, как только произошла любая смена. `POST /auth/confirm-email-change` ставит
адрес, `email_verified = True`, чистит флаги баунса, отзывает refresh-семейства И чистит куки
текущего браузера (отзыв семейства оставляет уже выданный access-токен живым до истечения — то же
поведение, что у reset-password).

**Альтернативы.**

- *Ставить пометку по факту ошибки отправки, без вебхука.* Не работает: Resend принимает письмо
  (202) и узнаёт о bounce от принимающего сервера через секунды-минуты. Синхронно этого не видно.
- *SMTP-проверка ящика (`VRFY` / `RCPT TO` без `DATA`).* Отвергнуто: половина провайдеров отвечает
  «ok» на всё (catch-all), Gmail/Outlook режут такие соединения, а исходящий 25 порт с прод-сервера
  обычно закрыт. Дало бы ложную уверенность и репутационный риск.
- *Считать SERVFAIL отказом.* Отвергнуто: домен с временно сломанными NS — это живой домен, и его
  владелец не должен терять регистрацию из-за пятиминутной аварии у своего DNS-провайдера.
- *`hits + 1` на каждый вебхук, включая ретраи.* Отвергнуто: счётчик перестал бы означать
  «сколько раз адрес реально отказал».
- *Уведомлять старый адрес о смене.* Отвергнуто сознательно: по постановке задачи это адрес,
  письма на который не доходят. Письмо «ваш email изменён» туда уйдёт в тот же Bounced и только
  ухудшит репутацию отправителя. Владельца предупреждает плашка в интерфейсе, а не письмо.
- *Добавить поля баунса в `UserOut`.* Отвергнуто: `UserOut` вложен в `CourseOut.owner`, то есть
  ушёл бы каждому студенту курса. Введён наследник `MeOut`, отдаваемый только из `/auth/me`.

**Trade-offs.**

- **Существование ящика проверить невозможно.** MX-проверка говорит только о ДОМЕНЕ.
  `nobody@gmail.com` пройдёт регистрацию — у `gmail.com` есть MX — и отсеется уже вебхуком после
  первого письма. Это не недоделка, а граница метода: единственный способ узнать про конкретный
  ящик — отправить туда письмо.
- **Внутри Docker проверка по умолчанию бесполезна.** `/etc/resolv.conf` в контейнере указывает на
  встроенный резолвер 127.0.0.11, который на несуществующий домен отвечает SERVFAIL вместо
  NXDOMAIN — а SERVFAIL у нас fail-open. Поэтому появился `DNS_RESOLVERS` (список рекурсивных
  резолверов через запятую); с пустым значением проверка не отсеет ничего. Измерено: `dsa.da` через
  127.0.0.11 → таймаут/SERVFAIL, через 1.1.1.1 → NXDOMAIN.
- Fail-open означает, что при недоступном DNS отсев не работает вовсе. Это выбрано осознанно:
  ложный пропуск ловится вебхуком, ложный отказ — потерянный пользователь.
- Подавление адреса не снимается автоматически, если ящик починили. Снимет `email.delivered` (но
  писем туда больше не уходит) либо ручное удаление строки. Жалоба (`complaint`) не снимается
  событием `delivered` вообще — события приходят не по порядку, и поздний `delivered` не должен
  возвращать в рассылку того, кто нажал «спам».

## 67. Удаление аккаунта подтверждается только ссылкой на почту; OAuth-вход в окне soft delete не воскрешает аккаунт (2026-09-10)

**Контекст.** Удаление аккаунта требовало ввести пароль (`POST /users/me/delete`, схема с полем
`password`). У пользователей, зарегистрированных через Google или Яндекс, `hashed_password` пуст —
форма для них была нерабочей в принципе: проверка `verify_password` не могла пройти ни при каком
вводе. Вторая, независимая дыра: пока аккаунт лежит soft-deleted (окно 30 дней, §59), вход через
провайдера искал пользователя отфильтрованным запросом, не находил его и уходил в ветку C
(регистрация нового аккаунта) — а там `INSERT` падал на уникальном индексе `users.email`, потому
что адрес всё ещё занят удалённой строкой.

**Решение.**

*Один сценарий для всех.* Парольная ветка удалена целиком, вместе со схемой запроса, проверкой
хеша и сообщением «Неверный текущий пароль». Вместо неё два роута рядом с остальными
почтовыми сценариями (`routers/auth.py`): `POST /auth/request-delete` — залогинен, под CSRF,
всегда 204, шлёт одноразовую ссылку; `POST /auth/confirm-delete` — анонимный, принимает только
токен, делает soft delete и отзывает сессии. Токен — тот же `itsdangerous` с отдельной солью
`account-delete` и TTL 30 минут, одноразовость — `SET NX` по sha256 в Redis, как у
`email-release`/`email-change`. Ни новой таблицы, ни миграции.

*Почему ссылка, а не «пароль, а для OAuth — что-нибудь ещё».* Вторая ветка означала бы вторую
модель угрозы, второй набор сообщений об ошибке и второй путь в тестах ради одного и того же
результата. Владение почтовым ящиком — единственный фактор, который есть у ВСЕХ аккаунтов:
парольные подтверждают им сброс пароля, OAuth-аккаунты получают адрес от провайдера, который его
уже проверил. Ссылка заодно даёт то, чего пароль не давал: письмо приходит владельцу ящика, то
есть перехваченная сессия сама по себе аккаунт не удалит.

*Кулдаун.* Per-user ключ в Redis (`ACCOUNT_DELETE_COOLDOWN_SECONDS`, 60 с) поверх slowapi-лимита
по IP — та же связка, что у `resend-verification`. Кнопка на фронте блокируется на тот же срок,
константа продублирована в `account.vue` с комментарием, потому что сервер срок в ответе не
сообщает (`Retry-After` в 429-обработчике нет, §60).

*Генерация урока.* Проверка «идёт генерация» осталась на месте, но выполняется дважды: на запросе
ссылки (чтобы отказ пришёл сразу, а не через письмо) и на подтверждении (между ними могли запустить
новую генерацию).

*OAuth в окне восстановления.* `_user_by_email` теперь смотрит с `include_deleted`, а ветка A —
тоже с `include_deleted` вместо отфильтрованного запроса. Единственная проверка живости
(`_assert_usable`) отказывает soft-deleted аккаунту кодом `account_pending_deletion` — тем же, что
уже отдают `/auth/login` (403) и `/auth/register` (409). Callback уводит на `/login?oauth=0&reason=`,
и страница показывает существующий CTA «Восстановить аккаунт». Обезличенная строка (после
освобождения адреса или покупки времени) — отдельный случай: она отвечает `account_disabled`,
потому что предлагать восстановление там значит вести на ссылку, которая может только не сработать.

**Альтернативы.**

- *Оставить пароль, а для OAuth-аккаунтов подтверждать повторным входом у провайдера.* Отвергнуто:
  два флоу, два места для ошибок, и повторный вход через провайдера — это не подтверждение
  намерения, а всего лишь та же авторизация ещё раз.
- *Удалять сразу по кнопке, без второго фактора.* Отвергнуто: удаление отзывает все сессии и
  запускает 30-дневный отсчёт до обезличивания. Одна кнопка в перехваченной сессии не должна
  запускать эту цепочку.
- *Хранить в Redis отпечаток «актуального» токена, чтобы жила только последняя ссылка.* Отвергнуто:
  новое состояние ради сценария, где обе ссылки делают одно и то же. Токены остаются независимыми,
  и вторая ссылка умирает на проверке `deleted_at` сразу после того, как сработала первая.
- *Оставить эндпоинт на `/users/me/delete` и просто заменить тело запроса.* Отвергнуто: подтверждение
  анонимно (ссылку открывают в другом браузере или из почтового клиента), а весь остальной
  токен-по-почте живёт на `/auth/*`.
- *Проверять доставляемость адреса перед отправкой ссылки (как `resend-verification`).* Отвергнуто
  на этом шаге, см. `KNOWN_PROBLEMS.md`: у пользователя с забаунсившим адресом отказ на запросе
  ссылки означал бы полную невозможность удалить аккаунт.

**Trade-offs.**

- **Ломающее изменение.** `POST /users/me/delete` больше нет. Осознанно: у нас один клиент — SPA
  из этого же репозитория, а держать нерабочую для половины пользователей ветку «на всякий случай»
  дороже, чем убрать её.
- **Удаление стало двухшаговым и зависит от почты.** Если письмо не дошло, аккаунт не удалить из
  интерфейса. Это цена универсальности: у OAuth-аккаунта других доказательств владения нет.
- **Адрес OAuth-аккаунта может быть не помечен `email_verified`.** Ссылку всё равно шлём: провайдер
  этот адрес уже подтвердил, и другого канала не существует.
- **Отправка письма — best-effort.** Недоступный брокер логируется и не влияет на 204, как и в
  остальных почтовых сценариях. Пользователь просто не получит ссылку и повторит запрос после
  кулдауна.
