...
толпа стримов уносила бы с собой логин. Поэтому pub/sub вынесен на отдельный 
пул (`get_pubsub_redis`, `REDIS_PUBSUB_MAX_CONNECTIONS=100`), а обычные команды 
остались на общем (`REDIS_MAX_CONNECTIONS=20`). Блast-радиус переполнения теперь 
ограничен самими стримами: новый стрим просто не открывается, и кабинет 
доживает на рефетче по фокусу. Потолок ≈ 100 одновременных вкладок на воркер 
(в проде `GUNICORN_WORKERS=4`); когда упрёмся — правильный шаг не «поднять 
число», а одна подписка `psubscribe('student:*:courses')` на процесс с 
раздачей по asyncio-очередям, тогда соединение будет одно независимо от числа 
студентов. (е) Нет автодополнения: чтобы добавить человека, нужно знать адрес целиком, опечатка 
