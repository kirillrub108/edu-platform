...
> Аудит реальной кодовой базы против докладных кандидатов (KNOWN_PROBLEMS, DECISIONS, ARCHITECTURE §9–10, DEPLOYMENT §7). Каждый пункт сверен с кодом, привязан к `file:line` и помечен статусом. Сгенерировано: **2026-06-15**; частичная ре-сверка **2026-07-30** (OBS-1, 2.6, 2.8, 3.6 закрыты кодом — помечены ✅ RESOLVED на месте). Источник истины — **код**; где доклад расходится с кодом, выигрывает код (см. «Документация vs код»).
>
...
|---|---|---|---|---|---|---|---|
| **OBS-1** | ~~В прод-compose нет Prometheus/Grafana/Flower~~ | `docker-compose.prod.yml` — сервисы `prometheus`/`grafana`/`flower` добавлены (Flower за `/flower` с basic-auth, Grafana за `/grafana`) | — | — | — | — | ✅ RESOLVED (2026-07-30) |
| **2.6** | ~~Redis без AOF; нет startup-реконсиляции~~ | прод-redis: `--appendonly yes` (`docker-compose.prod.yml:123`); `main.py:_reconcile_stuck_lessons` + `STUCK_LESSON_GRACE_MINUTES=120` помечает зависшие уроки `error` на старте | — | — | — | — | ✅ RESOLVED (2026-07-30); residual — KNOWN_PROBLEMS §2.6 |
| **2.8** | ~~Дисковые кеши растут без TTL/eviction~~ | beat-задачи `gc_disk_caches` (TTL+size-cap, mtime-LRU) и `gc_lesson_videos` в `tasks/purge_pipeline.py`; тюнинг `CACHE_GC_*`/`LESSON_VIDEO_GC_*` в `constants.py` | — | — | — | — | ✅ RESOLVED (2026-07-30) |
...
| **3.6** | ~~`VISION_SUMMARY_CONCURRENCY` — хардкод~~ | env-override в `config.py` + авто-вывод из CPU (`constants._derive_concurrency`, `CPU_BUDGET`) — весь concurrency-бюджет тюнится без пересборки | — | — | — | ✅ RESOLVED (2026-07-30) |
