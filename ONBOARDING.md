...
- 4–8 часов работы на 30 минут готового видео.
...
После раздела ты должен уметь объяснить:
- Зачем в проекте нужен Celery (а не «просто фоновый поток»)?
- Почему пайплайн «PPTX → MP4» не делается синхронно в HTTP-запросе?
- Какие 4 «режима создания урока» поддерживает система?
...
| Bizness-логика урока | `pages/lessons/[id].vue` (огромный файл — это «пульт управления уроком») |
...
- Access — 30 минут, refresh — 30 дней (см. `.env`).
...
- При 401 фронт **очищает localStorage и редиректит на /login** (refresh-флоу с автоматическим обновлением **не реализован** — это технический долг!).

⚠ **Дыра:** SHA-256 → bcrypt — это типичный «pre-hash перед bcrypt» хак, чтобы обойти 72-байтный лимит bcrypt. Он работает, но добавляет уязвимость к **password-shucking** (если у злоумышленника есть отдельная база sha256-хэшей этих паролей, он сразу получает плоский bcrypt-инпут). Для прод-использования стоит либо ограничить длину пароля при регистрации до 72 байт, либо перейти на argon2id.
...
| **Загруженные файлы не сканируются** | `services/storage_service.py` | можно загрузить .pptx со встроенным макросом → LibreOffice его выполнит? | в headless mode макросы отключены, но дополнительный sanity-check не помешает |
| **Полное содержимое .docx через `python-docx`** | `routers/uploads.py` | XML-bomb / XXE возможен в malformed файлах | использовать defusedxml или ограничить памятью |
...
| **`StatusBadge` принимает только 6 статусов**, новый статус → unknown badge | визуальный баг |
| **`task_status` для FAILURE не возвращает traceback** | дебажить пользовательские ошибки тяжело |
