...
/**
 * Shared lesson view: the student lesson page body, reused by the teacher's
 * «view as student» preview via the `preview` prop. In preview mode all data
 * comes from owner endpoints, «пройдено» lives in the in-memory preview store
 * and every write-path (attempts, submissions, comments) is disabled.
 */
...
const activeModule = computed(() =>
  props.preview
    ? previewStore.findModuleOfLesson(lessonId.value)
    : studentStore.activeCourse?.modules.find((m) =>
        m.lessons.some((l) => l.id === lessonId.value),
      ) ?? null,
)

const courseTitle = computed(() =>
  props.preview ? previewStore.course?.title : studentStore.activeCourse?.title,
)
...
const canDeleteComment = (c: Comment): boolean =>
  !props.preview && !!auth.user && c.author.id === auth.user.id

// In preview the panel is read-only: no edits either.
const canEditComment = computed(() =>
  props.preview ? () => false : undefined,
)
...
const loadLesson = async () => {
  ...
  try {
    if (props.preview) {
      if (previewStore.courseId !== courseId.value) {
        await previewStore.fetchCourse(courseId.value)
      }
      const allLessons = previewStore.course?.modules.flatMap((m) => m.lessons) ?? []
      if (allLessons.length && !allLessons.find((l) => l.id === lessonId.value)) {
        await navigateTo(`/courses/${courseId.value}/preview`, { replace: true })
        return
      }
      // Owner endpoint; lesson.video_url is exactly what a student would get —
      // it is only ever synced to the *published* LessonVideo (or direct upload).
      const lesson = await apiFetch<FullLesson>(`/lessons/${lessonId.value}`)
      fullLesson.value = {
        id: lesson.id,
        title: lesson.title,
      ...
      isCompleted.value = previewStore.isCompleted(lessonId.value)
    } else {
...
const onVideoUrlExpired = async () => {
  try {
    fullLesson.value = props.preview
      ? await apiFetch<FullLesson>(`/lessons/${lessonId.value}`)
      : await apiFetch<FullLesson>(`/students/lessons/${lessonId.value}`)
...
const markComplete = async () => {
  ...
  if (props.preview) {
    // Dry-run: local state only, wiped on preview exit.
    previewStore.markCompleted(lessonId.value)
        ...
        >
          <div class="mx-auto w-full max-w-[720px]">
            <LessonPlayer
              :lesson="fullLesson"
              :preview="preview"
              @video-url-expired="onVideoUrlExpired"
            />
          </div>

          <div
            class="bg-white border border-gray-100 rounded-2xl p-5 space-y-3"
            :class="lessonHiddenFromStudents && 'opacity-60'"
          >
            <h1 class="text-xl font-semibold text-gray-900 flex items-center gap-2 flex-wrap">
              {{ fullLesson.title }}
              <span
