...
**CI/CD is automated (since 2026-08-14):** `.github/workflows/ci.yml` runs `lint` + `test` (`pytest -m "not slow"`, `--cov-fail-under=70`) + `frontend` (`npm ci` + `npm run test` via vitest, working-directory `frontend`) → `deploy` (push to `master` only, `needs: [test, frontend]`). `deploy` SSHes to the prod server and runs `deploy/deploy.sh <short_sha>`, which builds sha-tagged images **on the server** (no registry — rejected as unneeded infra for a 1-server deployment, see `docs/DECISIONS.md` §46), dumps the DB via `db_backup` only if `alembic current` ≠ `alembic heads`, recreates containers, smoke-tests `/health` + `/docs`, and **auto-rolls back** to `last_good_sha` on failure (job stays red either way). Don't add a second deploy path (e.g. resurrecting `build-and-push` to a registry) without updating this script.
