...
- `backend/` — FastAPI + Celery (Python 3.13). Routers are thin, services are fat (LLM, TTS, video, storage, vision, auth, billing, quiz, assignment, analytics, visibility). Long jobs (PPTX→MP4 pipeline, vision analysis, quiz generation, payment settlement, soft-delete purge, disk GC, email) run in Celery.
...
Это правило про локальную (хост + контейнеры) среду. На GitHub Actions раннере (`.github/workflows/ci.yml`, job `frontend`) `npm ci` — чистая одноразовая Linux-среда без общего `node_modules`, поэтому там он корректен.
...
```bash
# Full stack: postgres, redis, backend, four Celery workers
...
# flower :5555) and frontend. No silero-tts service anymore (removed
# 2026-08-12) — see the TTS paragraph below before you touch TTS_PROVIDER.
...
External dependency the containers cannot provide: a reachable **LLM + vision provider**. The default in `.env.example` is **Polza AI** (cloud, OpenAI-compatible) for both text and vision. Alternatives are **Ollama on the host** with `qwen3` (text) and `qwen2.5vl:7b` (vision), reached via `host.docker.internal:11434` — pull those models if you point `LLM_BASE_URL`/`VISION_OLLAMA_BASE_URL` back to local Ollama — or **Yandex AI Studio** (`ai.api.cloud.yandex.net`, same OpenAI-compatible code path). Either way, without a working provider, video generation fails at the LLM-split or vision-summary step.

**TTS gotcha:** `.env.example` still defaults `TTS_PROVIDER=silero`, but the `silero-tts` container was removed from `docker-compose.yml`/`docker-compose.prod.yml` on 2026-08-12 (non-commercial license) — with the default as-is, the first video generation fails with "Silero TTS request failed" (connection refused). Set `TTS_PROVIDER=polza` or `TTS_PROVIDER=yandex` (SpeechKit v3, the prod default in `.env.prod.example`) instead; self-hosting `silero-tts` still works but is now manual (image `navatusein/silero-tts-service`, no compose wiring). See `docs/KNOWN_PROBLEMS.md` §5.6 and `docs/DECISIONS.md` §15/§45.
...
`docker-compose.yml` runs a dedicated worker per queue: `celery_video` (queue=`video`, c=`${VIDEO_CONCURRENCY:-2}`), `celery_vision` (queue=`vision`, c=1), `celery_quiz` (queue=`quiz`, c=2), and `celery_email_worker` (queue=`celery_email`, c=2). Queues/routing are declared in `app/celery_app.py`; tasks live in `app/tasks/` (`video_pipeline`, `vision_pipeline`, `quiz_pipeline`, `purge_pipeline`, `email_pipeline`, `payment_pipeline`) and must be in its `include=[...]` list. When adding a new task, route it to the right queue or no worker will pick it up. **Beat** is embedded in `celery_quiz` via `--beat` and drives **four** periodic jobs (all routed to `quiz`): `purge_soft_deleted` (03:00), `reconcile_pending_payments` (every 15 min — backstop for stuck YooKassa payments), `gc_disk_caches` (04:00) and `gc_lesson_videos` (04:30). Exactly one beat must run cluster-wide, so don't add `--beat` to another worker or replicate that service. On startup the backend also marks lessons stuck >120 min in `analyzing`/`processing` as `error` (`main.py:_reconcile_stuck_lessons`) — that's why "stuck" lessons self-heal after a restart.
...
`STORAGE_BACKEND` (`config.py`) switches between `local` and `s3` (Yandex Object Storage / S3-compatible); `services/storage_service.py` abstracts both. In `local` mode everything (uploaded PPTX, generated PNG/WAV/MP4, caches, attachments) lives in `backend/storage/`, bind-mounted into the backend and every celery container, and served with HMAC-signed URLs (`services/signed_url_service.py`). **Who serves `/files/*` depends on `SERVE_STATIC_VIA_NGINX`:** dev (false) → FastAPI's `files.router`; prod (true) → nginx serves the bytes directly from the shared volume and FastAPI only mounts `files.internal_router` to verify the signature. The files router is registered **only when `STORAGE_BACKEND == "local"`**. Cache dirs `slides_cache/`, `summaries_cache/`, `tts_cache/` (per-slide WAV) and `tts_chunk_cache/` (per-chunk WAV) are keyed by content hash — deleting them is safe and just forces re-rendering. `slides_cache`/`summaries_cache` are GC'd nightly by the `gc_disk_caches` beat job (TTL + size cap, recency = mtime bumped on cache hit — `CACHE_GC_*` in `constants.py`); the two TTS caches are **not** GC'd yet and grow unbounded.

### Video pipeline concurrency
`app/tasks/video_pipeline.py` runs two thread pools in parallel (TTS pool + FFmpeg encoder pool) and chains them with `as_completed` so encoding slide *k* starts the moment its WAV is ready. If you touch this file, preserve the streaming relationship — going back to "TTS all → encode all" roughly doubles pipeline latency. Pool sizes are **auto-derived from CPU count** (`constants._derive_concurrency`, capped at 12 cores) with verbatim env overrides in `config.py` (`CPU_BUDGET`, `TTS_WORKERS`, `ENCODE_WORKERS`, `VIDEO_CONCURRENCY`, `VISION_SUMMARY_CONCURRENCY`). **Invariant (historical):** the code comment in `constants.py` still says `TTS_WORKERS` is read by docker-compose as the Silero container's `NUMBER_OF_THREADS` — that stopped being true when `silero-tts` was removed from compose (2026-08-12); nothing in `docker-compose.yml` reads `TTS_WORKERS` anymore except the backend/celery pool itself. Only relevant again if you self-host `silero-tts` and wire `NUMBER_OF_THREADS` to it by hand. Other tunables (`SILERO_MAX_CHARS`, `SLIDE_DPI`, `MAX_SCRIPT_BYTES`) stay in `app/constants.py` — change them there, not inline. The pipeline also checkpoints synthesized slides to Redis (`job:{lesson_id}:checkpoint`) and keeps `work_dir` + checkpoint on failure/cancel so a re-run resumes — don't "clean up" the conditional `rmtree` in the `finally`, and support cooperative cancel (`cancel-generation` sets a flag the task checks at checkpoints).

### Lesson depth (`detail_level`), not a duration input
There is no `target_duration` field — an earlier iteration tried exactly that, and a short deck (3 slides, 15–20 min target) made the vision LLM demand ~1000 words/slide and degenerate into repeated punctuation instead. `Lesson.detail_level` (`brief`/`auto`/`high`, default `auto`) is the only knob; expected duration and price are **derived** from it (`services/duration_service.py`), never the other way round. In auto-mode it's read at **analysis** time (bounds the vision LLM's words-per-slide budget, `DETAIL_LEVEL_BODY_WORDS`); in manual mode (`presentation_and_text`) it's read at **generation** time, rewriting the author's own script via an SSML system-prompt override in `llm_service.py` (`brief` compresses, `high` elaborates, `auto` is verbatim and has no override at all — not an empty one). If you touch either prompt path, keep `duration_service.detail_ratio` as the single source of the word-budget ratio: the billing estimate reads the same function, so price and generated length can't drift apart. See `docs/DECISIONS.md` §57.
...
`LessonMaterial` (files) and `LessonNote` (markdown text) — both classes live in `models/lesson_material.py`, there's no separate `lesson_note.py` — let a teacher attach reference material to a lesson; no AI involved. One router (`routers/lesson_materials.py`) serves both roles, reusing the existing `require_lesson_access`/`get_owned_lesson` gates rather than a teacher/student split like assignments. Files are only stored, never parsed, with their own extension/size caps in `constants.py` (`LESSON_MATERIAL_*`) — separate from and not subject to the assignment `ATTACHMENT_*` retention sweep. See `docs/DECISIONS.md` §47.

### Billing, credits & quotas
Spend is governed by **credits**, not request quotas. `services/billing_service.py` is the source of truth: AI ops `RESERVE` credits up front then charge/release on completion; flat costs live in `CREDIT_WEIGHTS` and video is priced by formula (`VIDEO_*_BASE_CREDITS`, `TTS_CHARS_PER_CREDIT`, `estimate_video_text/auto`). Free accounts get a **lifetime trial** (`TRIAL_LECTURES`/`TRIAL_QUIZZES`, tracked in `usage_counters` with `period_key='lifetime'`) instead of welcome credits. Plans (`PLAN_CONFIGS`) grant a monthly allowance with `CREDIT_CARRYOVER_RATIO`; one-time top-ups come from `CREDIT_PACKAGES`. Payments go through **YooKassa** (`services/yookassa_service.py`, `routers/billing.py`, `tasks/payment_pipeline.py`): the webhook verifies the source IP (CIDR allowlist, `services/webhook_security.py`), never trusts the notification body, and only enqueues `process_yookassa_payment` (queue `quiz`, priority 0) before returning 200; the task re-fetches the payment from YooKassa's API and settles it through the single `_settle_payment` path (row-locked, double-credit-safe). The beat job `reconcile_pending_payments` re-drives payments stuck in `pending` through the same path — never add a second settlement code path. Admin grant/renewal endpoints (`/billing/admin/*`) are gated by a shared secret in the `X-Admin-Token` header (`require_admin`), not a UserRole; an empty `ADMIN_API_TOKEN` disables them. For one-off manual grants there's also `docker-compose exec backend python -m app.scripts.grant_credits <email> <amount> [--reason ...]`, a sync CLI wrapping `billing_service.sync_grant_credits` — separate path from the HTTP admin endpoints, useful when `ADMIN_API_TOKEN` isn't configured. The `generation_usage` journal records provider ₽ cost for margin tracking and feeds the Prometheus cost collector. Two more operations run through the same ledger: `QUIZ_GRADE` (LLM-grading a student's open answer — free up to `AI_GRADING_FREE_ANSWERS_PER_MONTH` per teacher per month, a flat technical floor outside `PLAN_CONFIGS`, not a plan tier) and `RETENTION_EXTEND` (paying to keep a graded submission's attachments past the default retention window, priced by attachment size via `estimate_retention_extension`). `POST /quiz/ai-review` (a teacher QA-checking their own quiz before publishing) is the opposite case: it used to be billed, but free-plan accounts have no balance for it and got a 402 on the first click, so it's now **always free** (`CREDIT_WEIGHTS["ai_review"] = 0`) — don't reintroduce a charge there without reading `docs/DECISIONS.md` §48–49 first.

### Quizzes, grading & assignments
**Quizzes** (`routers/quiz_teacher.py` / `quiz_student.py`, `services/quiz_service.py`, `tasks/quiz_pipeline.py`): teachers AI-generate or hand-author questions (8 `QuestionType`s: `single_choice`/`multiple_choice`/`true_false`/`short_answer`/`essay`/`matching`/`ordering`/`fill_blank`); generation runs on the `quiz` queue and is credit-charged. Student open-answer grading is **free LLM grading** (`services/grading_service.py`) with anti-abuse caps (`GRADING_MAX_ANSWER_CHARS`, `GRADING_MAX_ATTEMPTS_PER_QUIZ_PER_DAY`). **Assignments** (`routers/assignment_teacher.py` / `assignment_student.py`, `services/assignment_service.py`): teacher-set text tasks, student file submissions, grades, and a private per-submission message thread. Attachments are only *stored*, never parsed server-side (avoids XXE/zip-bomb); the MIME/extension whitelist and size/count caps live in `constants.py` (`ATTACHMENT_*`). Submission files auto-purge `ATTACHMENT_RETENTION_DAYS_AFTER_GRADED` days after grading via `purge_pipeline`. Grades roll up in `services/gradebook_service.py` (`routers/gradebook.py`); per-class/per-lesson quiz metrics (attempts, avg score, pass rate, per-student results with teacher override) live in `services/analytics_service.py` (`routers/analytics.py`).
...
### Course archiving
Archiving a course (`DELETE /courses/{id}` → soft-delete) is **teacher-facing only** — it does not revoke access for students already enrolled (the visibility rule above never checks `deleted_at`); it only blocks *new* enrollment (404, same as a draft) and drops the course from the catalog/preview. `purge_soft_deleted` (`tasks/purge_pipeline.py`, `_course_purge_guard`) permanently skips any archived course that has at least one `Enrollment`, no matter how old `deleted_at` is — the FK cascade `courses → modules → lessons → enrollments/lesson_progress`/quiz attempts/submissions would otherwise erase a student's own grades and history along with the course. Storage for such courses (video, PPTX, attachments) is therefore retained indefinitely; that's accepted, not a bug to "fix" by tightening the cascade. See `docs/DECISIONS.md` §51.
...
### Notifications
`services/notification_service.py` is the **single entry point** for lesson-generation notifications, both in-app and email — don't add a second ad-hoc `send_email.delay(...)` call for a new event; route it through here instead. Each event is delivered **urgent** (immediate) or **digest** (batched), and urgent email is **presence-gated**: it's skipped if the recipient already has a live SSE connection open for that lesson, tracked in the Redis sorted set `notify:presence:lesson:{id}` (written by the async progress-stream route, read by the sync Celery task — cross-process by design). Per-user delivery preferences are plain columns on `User`, not a separate settings table. See `docs/DECISIONS.md` §54.
...
LLM/vision provider is set in `.env` (`LLM_BASE_URL`, `LLM_MODEL`, `LLM_API_KEY`; `VISION_*`) — Polza AI (cloud default), Ollama, and Yandex AI Studio/YandexGPT all speak the OpenAI protocol, so `services/llm_service.py` / `services/vision_analysis.py` don't need changes; switching provider is an env edit. TTS is selected by `TTS_PROVIDER` inside `services/tts_service.py` — `silero` (self-host only, no longer in compose), `polza` (cloud), or `yandex` (SpeechKit v3, the prod default) — all three keep the same `synthesize()` signature so the pipeline doesn't change. Per-request `speed`/`pitch` overrides on `VideoGenerateRequest` only affect the `yandex` provider (SpeechKit hints) and persist per `LessonVideo` row.

## Production deployment

`docker-compose.prod.yml` is **self-contained — NOT an override** of the dev compose. Never run `-f docker-compose.yml -f docker-compose.prod.yml`: Compose merges lists, so dev bind-mounts would silently leak in. Code comes only from the built images. It adds over dev: **gunicorn** (uvicorn workers, no `--reload`) for the backend, `frontend/Dockerfile.prod` (`nuxt build` → node server), a one-shot **`migrate`** service (`--profile migrate`, run before `up`), a **`db_backup`** sidecar (periodic `pg_dump -Fc` → `db_backups` volume, retention `BACKUP_RETENTION_DAYS`), **nginx** rendered from `nginx/prod.conf.template` (the nginx image's envsubst substitutes only `${DOMAIN}` — `NGINX_ENVSUBST_FILTER`; there is no `prod.conf` anymore) serving `/files/*` directly + TLS + `X-Accel-Redirect` video streaming (`/protected-media/`), and a **certbot** profile (`deploy/init-letsencrypt.sh` reads `DOMAIN`/`CERTBOT_EMAIL` from `.env.prod`; systemd renew timer in `deploy/systemd/`). Storage is the shared `app_storage` named volume instead of a host bind-mount. Restore a backup with `... run --rm db_backup sh -c 'pg_restore -c -d "$PGDATABASE" /backups/<file>.dump'`. The web tier itself runs as two slots per service (since 2026-08-26) — `backend_blue`/`backend_green` and `frontend_blue`/`frontend_green` in `docker-compose.prod.yml` — only one live at a time. **There is no plain `backend`/`frontend` service in prod**; `up -d backend` fails with `no such service`. The green pair sits behind `profiles: ["green"]`, so any manual compose command touching it needs `--profile green` (`deploy/lib.sh`'s `compose()` wrapper already passes it). See `docs/DEPLOYMENT.md` §7.

**CI/CD is automated (since 2026-08-14) and zero-downtime (since 2026-08-26):** `.github/workflows/ci.yml` runs `lint` + `test` (`pytest -m "not slow"`, `--cov-fail-under=70`) + `frontend` (`npm ci` + `npm run test` via vitest, working-directory `frontend`) → `deploy` (push to `master` only, `needs: [test, frontend]`). `deploy` SSHes to the prod server and runs `deploy/deploy.sh <short_sha>`, which builds sha-tagged images **on the server** (no registry — rejected as unneeded infra for a 1-server deployment, see `docs/DECISIONS.md` §46), runs a migration-safety guard (`app.scripts.migration_guard` blocks a release whose migration is destructive — `DROP`/`RENAME`/`NOT NULL` — unless `DEPLOY_ALLOW_UNSAFE_MIGRATION=1`, since both slots share one DB mid-switch) and dumps the DB via `db_backup` only if `alembic current` ≠ `alembic heads`, boots the **idle slot**, waits for its healthcheck, flips the nginx upstream (`nginx -s reload`) onto it, smoke-tests `/health` + `/docs` through nginx, retags `:local`, recreates the Celery workers, then stops the old slot — traffic never drops. **Auto-rolls back** to `last_good_sha` on a late failure (job stays red either way). Manual operations against the prod stack (recreating a worker, switching slots by hand, editing `.env.prod`) have real gotchas — read `docs/DEPLOYMENT.md` §7.7 before running raw `docker compose` commands against it, especially: never `--force-recreate` a Celery worker without `--no-deps` (it depends on `redis`, which is also the auth/session store — recreating it drops every session). Don't add a second deploy path (e.g. resurrecting `build-and-push` to a registry) without updating this script.
